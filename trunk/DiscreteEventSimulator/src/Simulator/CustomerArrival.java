...
/**
 * CustomerArrival is a special SimEvent. At least one CustomerArrival in the
...
public class CustomerArrival extends SimEvent {
	...
	public void process() {
		...
		if (SimState.s.serverBusy == false) {
			...
			SimState.s.ec.insert(new CustomerArrival(SimState.s.now + iat));
		 ...
		 */
			...
			if (SimState.s.queue.size() < SimState.s.maxQueueSize) {
				...
				if ((SimState.s.preferablePlaces == -1) || (SimState.s.queue.size() < SimState.s.preferablePlaces)) {
					...
					SimState.s.ec.insert(new CustomerArrival(SimState.s.now + 2 * iat));
		...
		// ---- Update histogram
		// This histogram should probably be a continous histogram because the cc_qo counter is
		// also a time-based counter.
		CounterCollection.cc.waitingQueueLengthHistogram.count(SimState.s.queue.size());
